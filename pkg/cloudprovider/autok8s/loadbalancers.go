...
func (lb *loadbalancers) GetLoadBalancer(ctx context.Context, clusterName string, service *v1.Service) (status *v1.LoadBalancerStatus, exists bool, err error) {
	...
	klog.V(5).Info("GetLoadBalancer(%v)", clusterName)
	klog.V(5).Info("GetLoadBalancer ---")
	klog.V(5).Info("GetLoadBalancer service(%v)", service)
	klog.V(5).Info("GetLoadBalancer ---")
...
func (lb *loadbalancers) GetLoadBalancerName(ctx context.Context, clusterName string, service *v1.Service) string {
	...
	klog.V(5).Info("GetLoadBalancerName(%v)", clusterName)
	klog.V(5).Info("GetLoadBalancerName ---")
	klog.V(5).Info("GetLoadBalancerName service(%v)", service)
	klog.V(5).Info("GetLoadBalancerName ---")
	return ""
}
...
func (lb *loadbalancers) EnsureLoadBalancer(ctx context.Context, clusterName string, service *v1.Service, nodes []*v1.Node) (*v1.LoadBalancerStatus, error) {
	...
	test := v1.LoadBalancerIngress{IP: "10.10.10.1", Hostname: "test"} // to test use curl http://10.10.10.1 on worker
	LoadBalancerI = append(LoadBalancerI, test)

	var LoadBalancerS v1.LoadBalancerStatus
	LoadBalancerS = v1.LoadBalancerStatus{Ingress: LoadBalancerI}

	// LoadBalancerS = append(LoadBalancerS, stat)
	klog.V(5).Info("EnsureLoadBalancer(%v)", clusterName)
	klog.V(5).Info("EnsureLoadBalancer ---")
	klog.V(5).Info("EnsureLoadBalancer service(%v)", service)
	klog.V(5).Info("EnsureLoadBalancer ---")
	klog.V(5).Info("EnsureLoadBalancer nodes(%v)", nodes)
	klog.V(5).Info("EnsureLoadBalancer ---")
...
func (lb *loadbalancers) UpdateLoadBalancer(ctx context.Context, clusterName string, service *v1.Service, nodes []*v1.Node) error {
	...
	klog.V(5).Info("UpdateLoadBalancer(%v)", clusterName)
	klog.V(5).Info("UpdateLoadBalancer ---")
	klog.V(5).Info("UpdateLoadBalancer service(%v)", service)
	...
	klog.V(5).Info("UpdateLoadBalancer ---")
	klog.V(5).Info("UpdateLoadBalancer nodes(%v)", nodes)
	klog.V(5).Info("UpdateLoadBalancer ---")
	return nil
}
...
func (lb *loadbalancers) EnsureLoadBalancerDeleted(ctx context.Context, clusterName string, service *v1.Service) error {
	...
	klog.V(5).Info("EnsureLoadBalancerDeleted(%v)", clusterName)
	klog.V(5).Info("EnsureLoadBalancerDeleted ---")
	klog.V(5).Info("EnsureLoadBalancerDeleted - service(%v)", service)
	klog.V(5).Info("EnsureLoadBalancerDeleted ---")
	return nil
}
